...
class itemClass
{
	...
	itemClass() : name(), description(), type(objtype_no_type), color(-1), hit(0), damage(Dice()), dodge(0), defence(0), weight(0), speed(0), attribute(0), value(0), equipment(false), symbol('*'), twoHanded(false), pos(NULL_POS), item_stack(false)
	{	
	}
